...
## 스킬 설계 원칙 (수정 시 유지할 것)
1. 역할 분리: create-ssot/update-ssot=기획자(기획 질문만), create-task/implement-task=엔지니어(개발 질문만). 서로의 영역을 침범하는 지시를 넣지 않는다.
2. 문서 먼저: 모든 스킬은 질문·추론·구현 전에 `spec/STATE.md`(관제탑)에 기록. 이 공통 규칙 4줄은 6개 스킬에 동일하게 반복돼 있다 — 바꾸면 전부. 단 create-narrative만 3·4번에 예외 문구(산출물이 사람용 한국어 산문)가 있다.
